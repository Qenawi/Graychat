...
public class SignInActivity extends AppCompatActivity implements
 ...
 public  void add_user_data( )
 {
  ...
  void creat_user_friend_list()
  {
      ...
      String key2=  dpf.child("wallpaber").push().getKey();
      ContractAcc acc=new ContractAcc();
      dpf.child("wallpaber").child(key2).setValue(new WallPaberv(acc.get_username().getEmail(),"tmp"));
      String key3=  dpf.child("Status").push().getKey();
      dpf.child("Status").child(key3).setValue(new Statusm("tmp",acc.get_username().getEmail()));
      finishX();
  }
  ...
  {
      PG.setVisibility(View.GONE);
      Log.v(ContractDepug.PUBTAG,"res10000");
